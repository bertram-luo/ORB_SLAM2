...
void CtmsTracker::HaarFeature(Rect& _objectBox, int _numFeature)
...
void CtmsTracker::sampleRect(Mat& _image, Rect& _objectBox, float _rInner, float _rOuter, int _maxSampleNum, vector<Rect>& _sampleBox)
...
void CtmsTracker::sampleRect(Mat& _image, Rect& _objectBox, float _srw, vector<Rect>& _sampleBox)
...
void CtmsTracker::getFeatureValue(Mat& _imageIntegral, vector<Rect>& _sampleBox, Mat& _sampleFeatureValue)
{
...
void CtmsTracker::classifierUpdate(Mat& _sampleFeatureValue, vector<float>& _mu, vector<float>& _sigma, float _learnRate)
{
	Scalar muTemp;
	Scalar sigmaTemp;
...
void CtmsTracker::radioClassifier(vector<float>& _muPos, vector<float>& _sigmaPos, vector<float>& _muNeg, vector<float>& _sigmaNeg,
										 Mat& _sampleFeatureValue, float& _radioMax, int& _radioMaxIndex, cv::Mat& prob, vector<Rect>& _sampleRect)
{
	float sumRadio;
	_radioMax = -FLT_MAX;
	_radioMaxIndex = 0;
	...
	for (int j=0; j<sampleBoxNum; j++)
	{
		...
		Mat roi = prob(_sampleRect[j]);
		//prob.at<float>( _sampleRect[j].y + _sampleRect[j].height/2, _sampleRect[j].x + _sampleRect[j].width / 2) = sumRadio;
		if (sumRadio > 0){
		    radioPosSum += sumRadio;
		    roi += sumRadio;
		}
	...
	for(int j = 0; j < sampleBoxNum; j++){
	    float val = prob.at<float>(_sampleRect[j].y + _sampleRect[j].height/2, _sampleRect[j].x + _sampleRect[j].width / 2);
        //prob.at<float>( _sampleRect[j].y + _sampleRect[j].height/2, _sampleRect[j].x + _sampleRect[j].width / 2) = val > 0 ? val / _radioMax : 0;
        prob.at<float>( _sampleRect[j].y + _sampleRect[j].height/2, _sampleRect[j].x + _sampleRect[j].width / 2) = val > 0 ? val / radioPosSum : 0;

	}
}
void CtmsTracker::init(Mat& _frame, Rect& _objectBox)
{
...
void CtmsTracker::processFrame(Mat& _frame, Rect& _objectBox, int& radioMaxIndex, float& radioMax, cv::Mat& _prob)
{
    printf("entering process frame ctms\n");
	// predict
	...
	//int radioMaxIndex;
	//float radioMax;
    printf("entering radio classifier ctms\n");
	radioClassifier(muPositive, sigmaPositive, muNegative, sigmaNegative, detectFeatureValue, radioMax, radioMaxIndex, _prob, detectBox);
