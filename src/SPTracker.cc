    ...
    for(unsigned int i = 0; i < groundtruth_boxes.size(); ++i){
        ...
        //for(int row = 0; row < 2; ++row){
            //for(int col = 0; col < 40; ++col){
                //auto pixel = train_frames[i].warpimg_hsi.at<cv::Vec3f>(row, col);
                //printf("[%f, %f, %f]", pixel[0], pixel[1], pixel[2]);
            //}
            //printf ("\n");
        //}
...
void SPTracker::run(cv::Mat& frame, cv::Rect& new_location){
    ...
    //t1_update_info();
    //if (test.update_interval_num >= update_frequencyi && update_index_pre == update_incre_num){
      //  t1_update_app_model();
    //}
    t1_update_info();
...
void SPTracker::do_tracking(cv::Mat& frame){
    ...
    SLAM_DEBUG(" size [%d, %d] [%d, %d]", train_frames.back().warpimg_hsi.rows, train_frames.back().warpimg_hsi.cols, rows, cols);
    for(int row = 0; row < rows; ++row){
        for(int col = 0; col < cols; ++col){
            pro_image_w1.at<float>(row, col) = w1[*(test.labels + row * cols + col)];
        }
    }
    ...
    //for(int row = 0; row <= pro_image_w1_integral.rows - h; ++ row){
        //int ymin = row;
        //int ymax = row + h;
        //for(int col = 0; col <= pro_image_w1_integral.cols - w; ++ col){
            //int xmin = col;
            //int xmax = col + w;
            //double val = pro_image_w1_integral.at<float>(ymax, xmax)
            //- pro_image_w1_integral.at<float>(ymin, xmax) 
            //- pro_image_w1_integral.at<float>(ymax, xmin) 
            //+ pro_image_w1_integral.at<float>(ymin, xmin);
            //if (val > max_spt_conf){
                //max_spt_conf = val;
                //max_row =  row;
    ...
    SLAM_DEBUG("sample regions spt confidence calculation completed with rect [%d, %d, %f, %f]",  int(max_col + w / 2), int(max_row + h / 2), w, h);

    test.p[0] = max_col + w /2 + x1;
    test.p[1] = max_row + h /2 + y1;
    SLAM_DEBUG(" frame [%d] new object location [%f, %f, %f, %f], with confidence [%f]", frame_num, test.p[0], test.p[1], test.p[2], test.p[3], max_spt_conf);
    ...
    float occlusion_conf = 0.5 - max_spt_conf / test.p[2] / test.p[3];
    if (occlusion_conf > occlusion_rate){
        update_flag = false;
    } else {
        update_flag = true;
        test.warp_p = test.p;
    ...
    SLAM_DEBUG("#############check sum in do_tracking");
    for (int ptIdx = 0; ptIdx < test.sp_cl_hist[0].size(); ++ ptIdx){
        float sum = 0;
        for(int dimNo = 0; dimNo < numDim; ++ dimNo){
               sum += test.sp_cl_hist[dimNo][ptIdx];
        }
        if (abs(sum - 1) > 1e-1){
            printf("invalid sum in do_tracking %f", sum);
        }
    }
...
void SPTracker::t1_construct_appearance_model(vector<vector<float>>& train_sum_hist, int train_sp_sum_num){
    //for(int row = 0; row < train_sum_hist.size(); ++row){
        //for(int col = 0; col < train_sum_hist[row].size(); ++ col){
            //printf("%f ", train_sum_hist[row][col]);
        //}
        //printf("\n");
    //}
    //printf("end of train_sum_hist\n");
...
void SPTracker::t1_show_cal_cluster_wt(int frame_num, int cluster_sum, vector<int>& data2cluster, vector<SPTFrame>& frames, vector<pair<double, double>>& TrainCluster_Weight, vector<int>& sp_index_pre ){
    SLAM_DEBUG("%d cluster", cluster_sum);
